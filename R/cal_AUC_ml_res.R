      ...
      if (mode == "all") {
        ...
        if (num.b == num.a) {
          ...
          if (single_ml == "MCP") {
            ...
            feature.ac <- colnames(as.matrix(train_data[, -c(1:3)]))
            val_dd_list2 <- lapply(val_dd_list, function(x) {
              x[, c("OS.time", "OS", intersect(feature.ac, colnames(x)))]
            })

            rs <- lapply(val_dd_list2, function(x) {
              cbind(x[, 1:2], RS = as.numeric(predict(fit, X = as.matrix(x[, -c(1:2)]), type = "lp")))
            })
            roc.test <- returnRStoROC(rs.table.list = rs, AUC_time = AUC_time)
            AUC.rs[[names(res.by.ML.Dev.Prog.Sig$ml.res)]] <- roc.test
